...
function initSearchBox(map) {
  map.updateSearchBoxText = () => {
    ...
    map.searchBox.attr('placeholder', text);
  };
  ...
  map.searchBox.click(e => {
    e.stopPropagation();
  });

  map.searchBox.dropdown.add = feature => {
  ...
  map.searchBox.on('input', e => {
    ...
    const visibleMatches = matches.filter(match => match.isShown());

    $.each(visibleMatches.slice(0, 4), (index, match) => {
...
function loadFeatures(featureData) {
  ...
  $.each(featureData, function (index, featureData) {
    if (!featureData.name ||
        !featureData.type ||
